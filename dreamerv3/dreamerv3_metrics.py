...
LATENT_DIR = ROOT_DIR.parent / 'maanyacb' / 'rq3_outputs' / 'dreamerv3'
ID_LATENTS_PATH = LATENT_DIR / 'id_latents.npy'
...
id_lat = np.load(str(ID_LATENTS_PATH)).astype(np.float32)     # [N, T, D]
ood_lat = np.load(str(OOD_LATENTS_PATH)).astype(np.float32)   # [N, T, D]
...
# Match original script expectation:
# emb        : [N, D] pooled per-video embeddings
# frame_emb  : [N, T, D] per-frame embeddings for temporal MSE gap
emb_pos = id_lat.mean(axis=1)
emb_imp = ood_lat.mean(axis=1)
emb = np.concatenate([emb_pos, emb_imp], axis=0).astype(np.float32)
frame_emb = np.concatenate([id_lat, ood_lat], axis=0).astype(np.float32)
...
json_out = OUTPUT_DIR / 'dreamerv3_metrics.json'
csv_out  = OUTPUT_DIR / 'dreamerv3_metrics.csv'
txt_out  = OUTPUT_DIR / 'dreamerv3_metrics.txt'
...
with open(str(txt_out), 'w', encoding='utf-8') as f:
    f.write(f"Model: dreamerv3  |  embed_dim={embed_dim}\n")
