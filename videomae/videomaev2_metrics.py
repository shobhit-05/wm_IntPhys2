...
print(f"Loading joined labels: {JOINED_CSV}")
...
types = [
    ((r.get('type') or r.get('type_raw') or r.get('possible_or_impossible') or '').strip())
    for r in aligned_rows
]
difficulties = [((r.get('difficulty') or r.get('Difficulty') or '').strip()) for r in aligned_rows]
cameras = [((r.get('camera') or r.get('Camera') or '').strip()) for r in aligned_rows]
...
json_out = OUTPUT_DIR / 'videomaev2_metrics.json'
csv_out  = OUTPUT_DIR / 'videomaev2_metrics.csv'
txt_out  = OUTPUT_DIR / 'videomaev2_metrics.txt'
...
with open(str(txt_out), 'w', encoding='utf-8') as f:
    f.write(f"Model: videomaev2  |  embed_dim={embed_dim}\n")
